...
class BaseRateLimitDecorator:
  ...
  def __call__(
  ...
  ) -> Callable[[Union[Request, View]], Coroutine[Any, Any, Response]]:
    ...
    async def wrapper(ctx: Union[Request, View]) -> Response:
      ...
      if any([
          ip_address in ipaddress.ip_network(net) for net in self.exempt_ips
      ]):
      ...
      if not await self.moving_window.test(self.item, db_key):
        if self.error_handler is not None:
          if asyncio.iscoroutinefunction(self.error_handler):
            r = await self.error_handler(request,
                                         RateLimitExceeded(self.amount))
            ...
            if isinstance(r, Allow):
              return await func(request)
            return r
        data = json.dumps({"error": f"Rate limit exceeded: {self.amount}"})
        response = Response(text=data,
                            content_type="application/json",
                            status=429)
        response.headers.add("error", f"Rate limit exceeded: {self.amount}")
        return response
